...
DEFAULT_SOURCE_ROOT = Path("src/dotmd")
...
def _function_metrics_from_report(
    ...
    for raw_path, file_data in files.items():
        ...
        block_items = _qualname_from_block_list(blocks)
        qualname_counts = Counter(qualname for qualname, _block in block_items)
        for qualname, block in block_items:
            ...
            key_qualname = (
                f"{qualname}@L{block.lineno}" if qualname_counts[qualname] > 1 else qualname
            )
            metrics.append(
                FunctionMetric(
                    key=f"{relative_path}::{key_qualname}",
                    path=relative_path,
                    qualname=qualname,
