...
def test_index_phase_treats_existing_indexes_as_applied(tmp_path: Path) -> None:
    from dotmd.ingestion import migrate_surreal as migrate_module  # type: ignore[import-not-found]
    ...
    class _ExistingIndexConnection:
        def query(self, _statement: str):  # type: ignore[no-untyped-def]
            nonlocal calls
            calls += 1
            if calls == 1:
                raise RuntimeError("The index 'embeddings_strategy_chunk_model_idx' already exists")
            return []

    migrate_module._write_index_phase(
        checkpoint,
        report=report,
        connection=_ExistingIndexConnection(),
        progress_path=progress_path,
    )
